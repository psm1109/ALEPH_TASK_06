...
- 이번 주 학습 기록의 체크는 날짜별 완료 기록을, 아래 시간은 실행 기록의 날짜별 실제 소요 시간 합계를 각각 표시합니다.
- 열린 화면에서는 서울 시간 기준 다음 날 0시에 완료 체크가 풀립니다. 앱을 닫아 둔 경우에는 다시 열 때 지난날의 완료 상태를 초기화합니다. 같은 할 일을 다시 완료하면 새 날짜의 체크가 기록됩니다.
...
1. Supabase 프로젝트의 SQL Editor에서 [`supabase/schema.sql`](supabase/schema.sql)을 실행합니다. 이 SQL은 RLS뿐 아니라 로그아웃된 세션을 Data API에서 즉시 거절하는 사전 요청 검사도 설정합니다.
   ...
   기존 PDS Diary 데이터베이스에는 날짜별 완료 체크를 사용하기 전에 [`supabase/daily-completion.sql`](supabase/daily-completion.sql)을 추가로 실행합니다. 기존 완료·실행 기록은 보존하며, 과거에 중복 제한으로 저장되지 못한 완료 날짜는 복구할 수 없습니다.
...
- 서울 날짜가 바뀌면 할 일의 현재 체크가 풀리고 지난날 학습 기록의 체크는 유지됩니다.
