...
- 이번 주 학습 기록의 오늘 체크는 계획 탭의 현재 체크 상태와 즉시 연동됩니다. 체크를 풀면 오늘 기록의 체크도 사라집니다. 아래 시간은 실행 기록의 날짜별 실제 소요 시간 합계입니다.
- 서울 시간 기준 날짜가 바뀌면 그 직전에 남아 있던 체크만 완료 이력으로 확정하고 새날의 체크를 풉니다. 앱을 닫아 둔 경우에는 다시 열 때 같은 규칙을 적용합니다.
...
- 할 일 수는 현재 계획에 연결된 할 일을 집계합니다. 완료 수는 집계 기간에 남은 날짜별 완료 기록의 합계이며, 같은 할 일을 여러 날 완료했다면 날짜마다 1건씩 셉니다. 막힘 수는 현재 자료에서 집계합니다. 지연 수는 서울 날짜별 미완료 기록의 합계입니다. 같은 할 일이 여러 날 미완료였다면 날짜마다 1건씩 기록합니다.
...
- Plan에서 오늘 완료 체크를 해제하면 오늘의 완료 기록과 돌아보기 완료 수가 즉시 줄어듭니다. 지난 날짜의 완료 기록은 유지됩니다.
...
- 할 일을 완료하면 이번 주 학습 기록에 체크가 표시되고, 실행 기록을 저장하면 같은 칸 아래에 실제 시간이 별도로 표시됩니다.
- 오늘 완료 체크는 해제할 수 있고, 서울 날짜가 바뀌면 마지막 체크 상태를 이력으로 남긴 뒤 새날의 체크가 풀립니다.
