...
- 예상 시간은 현재 할 일의 일일 예상 시간 합계에 계획 시작일부터 오늘까지 시작일과 오늘을 포함한 경과 일수를 곱해 표시합니다. 근거 기록에는 각 할 일 제목과 일일 예상 시간을 함께 표시합니다. 실제 시간은 집계 기간의 실행 기록을 날짜별로 합산하고, 근거 기록에는 날짜별 실행 완료 내역과 각 실제 소요 시간을 표시합니다. `실제 − 예상` 차이도 표시합니다.
